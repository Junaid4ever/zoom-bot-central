# ============================================
# ZOOM BOT CENTRAL SERVER - Railway (COMPLETE)
# ============================================
import os
import uuid
...
# IMPORTANT: asgi_app hi Railway pe use hoga
...
async def start_bots(req: StartBotRequest):
    if req.bot_count < 1:
        raise HTTPException(status_code=400, detail="bot_count must be >= 1")
    ...
    if not assigned:
        raise HTTPException(
            status_code=503,
            detail="No free capacity. Start Colab worker first."
        )
...
async def dashboard():
    # pehle file try, nahi mili to embedded HTML
    try:
